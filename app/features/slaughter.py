...
async def start_slaughter(user: discord.User) -> None:
    ...
    try:
        reaction, user = await bot.wait_for(
            "reaction_add",
            check=lambda reaction, user: user == user and reaction.message == message,
        )
...
async def handle_slaughter_reaction(
    ...
    theme = (
        session.query(Theme)
        .filter(Theme.theme == reaction.message.content.split(": ")[1])
        .first()
    )
    if not theme:
        return
